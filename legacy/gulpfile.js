...
var paths = {
  ...
  tmpHTML: 'public/**/*.html',
  tmpCSS: 'public/assets/css',
  tmpJS: 'public/assets/js',
  tmpPhp: 'public/assets/php',
  tmpImages: 'public/assets/img',
  tmpmedia: 'public/assets/media',
  tmpFonts: 'public/assets/fonts/',

  dist: 'dist',
  distIndex: 'dist/index.html',
  distCSS: 'dist/assets/css/',
  distJS: 'dist/assets/js/',
  distPhp: 'dist/assets/php',
  distImages: 'dist/assets/img',
  distmedia: 'dist/assets/media',
...
gulp.task(
  'views-rebuild',
  gulp.series('views', function(done) {
    browserSync.reload();
    done();
  })
);
...
gulp.task('fonts', function() {
  return gulp.src(paths.srcFonts).pipe(gulp.dest(paths.tmpFonts));
});
gulp.task('php', function() {
  return gulp.src(paths.srcPhp).pipe(gulp.dest(paths.tmpPhp));
});
gulp.task('media', function() {
  return gulp.src(paths.srcmedia).pipe(gulp.dest(paths.tmpmedia));
});
...
gulp.task(
  ...
  gulp.series('views', function() {
    return gulp.src(paths.tmpHTML).pipe(gulp.dest(paths.dist));
  })
);
gulp.task(
  'html:dist-min',
...
gulp.task(
  ...
  gulp.series('scripts', function() {
    return gulp.src(paths.tmpJS + '/**/*').pipe(gulp.dest(paths.distJS));
  })
);
...
gulp.task(
  'php:dist',
  gulp.series('php', function() {
    return gulp.src(paths.tmpPhp + '/**/*').pipe(gulp.dest(paths.distPhp));
  })
);
gulp.task(
  'media:dist',
  gulp.series('media', function() {
    return gulp.src(paths.tmpmedia + '/**/*').pipe(gulp.dest(paths.distmedia));
  })
);
/*---------------------------------------
...
gulp.task(
  ...
  gulp.series('fonts', function() {
    return gulp.src(paths.tmpFonts + '/**/*').pipe(gulp.dest(paths.distFonts));
  })
);
...
gulp.task(
  'make-demo',
  gulp.series(['clean:demo-folder'], function() {
    return gulp.src(bases.dist + '**').pipe(gulp.dest(bases.demoFolder));
  })
);
...
gulp.task(
  'demo',
  gulp.series('production-min', 'previews-resize', 'img-compress', 'make-demo')
);
...
gulp.task(
  'tf-zip',
  gulp.series(['clean:tf-zip'], function() {
    return gulp
      .src(tf.zipSrc + '**')
...
gulp.task(
  ...
  gulp.series(
    ...
    function() {
      gulp.src('gulpfile.js').pipe(gulp.dest(tf.root));
      gulp.src('package.json').pipe(gulp.dest(tf.root));
      ...
      //development version
      gulp.src(bases.app + '**').pipe(gulp.dest(tf.app + 'src'));

      //production version
      gulp.src(bases.dist + '**').pipe(gulp.dest(tf.app_production));

      //Replace compress and placeholder images from dist to src folder
      del(tf.app + 'src/assets/img/**/*', { force: true });

      return gulp
        .src(tf.app + 'dist/assets/img/**/*')
...
gulp.task(
  'tf',
  gulp.series('tf-version', 'tf-zip', 'tf-upload', 'clean:tf', 'clean:tf-zip')
);
...
gulp.task(
  'deploy',
  gulp.series('default', 'demo', 'upload', 'clean:demo-folder')
);
